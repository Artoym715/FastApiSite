...
async def create_note(count: int):
    ...
    return {'Сообщение': f'Cоздано {count} рандомных пользователей.'}
...
async def create_note(count: int):
    for i in range(count):
        ...
        await db.database.execute(query)
    return {'Сообщение': f'Cоздано {count} рандомных товара.'}
...
async def create_note(count: int):
    for i in range(count):
        ...
        await db.database.execute(query)
    return {'Сообщение': f'Cоздано {count} рандомных заказа.'}
...
async def read_user(user_id: int):
    ...
    if user is None:
        raise HTTPException(status_code=404, detail="Пользователей не найдены!")
    return user
...
async def delete_user(user_id: int):
    ...
    await db.database.execute(query)
    return {'Сообщение': 'Пользователей удален!'}
